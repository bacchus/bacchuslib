...
void init_scene() {
    ...
    p2->center = vec3f(0.0f,0.25f, 0.0f);
    p2->radius = 0.25f;
...
mat3f setCamera(const vec3f& ro, const vec3f& ta, float rol) {
    vec3f zax = normalize(ta-ro);
    vec3f up = vec3f(std::sin(rol), std::cos(rol), 0.f);
    vec3f xax = normalize(cross(zax,up));
    vec3f yax = normalize(cross(xax,zax));
...
vec3f render(const vec3f& eye, const vec3f& dir) {
    vec3f bgcolor = vec3f(0.8, 0.9, 1.0);
    vec3f ligdir = vec3f(-0.6f, 0.7f, -0.5f);
    vec3f col = bgcolor;
    float t;
    ...
    if(p) {
        ...
        vec3f lig = normalize(ligdir);
        float dif = clamp(dot(nor,lig), 0.f, 1.f);
        vec3f brdf = vec3f(0.0);
        brdf += dif*vec3f(1.0f,0.9f,0.6f);
        col = col*brdf;
        col = mix(col, bgcolor, 1.0f-exp(-0.0005f*t*t));// fog
    }
...
void process(Image& img) {
    ...
    init_scene();
    vec3f eye = vec3f(1.0f, 1.0f, 3.5f);
    vec3f tar = vec3f(0.0f, 0.0f, 0.0f);
    mat3f view = setCamera(eye, tar, 0.0);
    ...
    for (fcord.y = 0; fcord.y < img.resolution().y; ++fcord.y) {
        for (fcord.x = 0; fcord.x < img.resolution().x; ++fcord.x) {
            ...
            img.pix(fcord) = rgb2i(255.f*render(eye, raydir));
        }
    }
}
