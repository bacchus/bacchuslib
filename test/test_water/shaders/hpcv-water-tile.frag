...
void main (void)
{
    ...
    vec2 ff =  abs(2.0*(fract(mytexFlowCoord)) - vec2(1.0)) - vec2(0.5);
    // take a third power, to make the area with more or less equal contribution
    // of more tile bigger
    ff = vec2(0.5) - 4.0*ff*ff*ff;
    ...
    // compensates for it
    vec2 ffscale = sqrt(ff*ff + (vec2(1.0) - ff)*(vec2(1.0) - ff));
    ...
    vec2 flowdir = sample.xy - vec2(0.5);
    ...
    sample = texture2D( flowMap, floor((mytexFlowCoord + vec2(0.5,0.0)))/ texScale ).rgb;
    flowdir = sample.b * (sample.xy - vec2(0.5));
    ...
    vec2 NormalT1 = texture2D(normalMap, rotmat * Tcoord - offset*1.06 + vec2(0.62)).rg ;

    // blend them together using the ff factor
    // use ff.x because this tile is shifted in the x-direction
    vec2 NormalTAB = ff.x * NormalT0 + (vec2(1.0) - ff.x) * NormalT1;
    ...
    flowdir = sample.b * (sample.xy - vec2(0.5));
    rotmat = mat2(flowdir.x, -flowdir.y, flowdir.y ,flowdir.x);
    NormalT0 = texture2D(normalMap, rotmat * Tcoord - offset*1.33 + vec2(0.27)).rg;
    ...
    vec2 NormalTCD = ff.x * NormalT0 + (vec2(1.0) - ff.x) * NormalT1;
    // NormalTCD = (NormalTCD - 0.5) / ffscale.x + 0.5;

    // now blend the two values together
    vec2 NormalT = ff.y * NormalTAB + (vec2(1) - ff.y) * NormalTCD;
    ...
    NormalT = (NormalT - vec2(0.5)) / (ffscale.y * ffscale.x);
