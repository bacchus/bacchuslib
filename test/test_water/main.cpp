...
public:
    WaterFilter(const std::string& vs, const std::string& fs)
        : Filter(vs,fs)
    {
        m_time.x = 0.f;
        m_mouse.x = 10;
...
public:
        ...
        : QGLWidget(parent)
    {
        ...
        /// opengl specific
        std::string vstr = file_readbuff("../shaders/simple.vert");
        std::string fstr = file_readbuff("../shaders/hpcv-water-tile.frag");
        m_filter = new WaterFilter(vstr, fstr);
